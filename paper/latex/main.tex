...
\begin{itemize}[leftmargin=2em, itemsep=0.2em]
  ...
  \item Native AVX2 SIMD kernels for binary GEMM, SSM scan, and 1-bit convolution, achieving hardware-native execution on any post-2013 x86 CPU.
...
We profile Wan~1.3B on an Intel Pentium Gold 7505 (2 cores, 4 threads, 3.5\,GHz, 16\,GB DDR4) using float16 precision. Input: batch size 1, 8 frames at $256 \times 256$ resolution (latent space: $4 \times 32 \times 32$). We run 2 warmup iterations followed by 5 timed iterations, reporting mean and standard deviation.
...
\subsection{AVX2 Native Kernels}
\label{sec:avx2}

We implement three C kernels using AVX2 SIMD intrinsics (256-bit vectors), targeting any x86 CPU manufactured since 2013:

\textbf{Binary GEMM.} Each \texttt{\_mm256\_xor\_si256} instruction processes 256 weight bits simultaneously. Popcount is computed via a nibble lookup table using \texttt{\_mm256\_shuffle\_epi8}, achieving approximately one popcount per cycle per 32-byte vector. Matrix operations are tiled to L1 cache size (80\,KB on the Pentium Gold).

\textbf{SSM scan.} The sequential scan (Eq.~\ref{eq:ssm_state}) is vectorized across the state dimension $d_{\text{state}}$ using \texttt{\_mm256\_fmadd\_ps} (fused multiply-add on 8 floats). With $d_{\text{state}}=16$, each timestep requires only 2 AVX2 iterations.
...
\textbf{Hardware.} Primary benchmark: Intel Pentium Gold 7505 (2 cores, 4 threads, 3.5\,GHz), 16\,GB DDR4 3200\,MHz (single channel), no discrete GPU. This represents the lower end of commodity hardware---a deliberate choice to establish a conservative performance floor.

\textbf{Software.} Python 3.9, PyTorch (CPU-only build), custom C kernels compiled with \texttt{gcc -mavx2 -O3 -march=native}. All code is available at \texttt{github.com/IshCPU-VideoGenLab}.
...
\begin{enumerate}[leftmargin=2em, itemsep=0.1em]
  ...
  \item \emph{+AVX2}: Native kernels, all else unchanged
...
\subsection{AVX2 Kernel Performance}
...
\begin{table}[h]
\centering
\caption{Primary benchmark hardware.}
\begin{tabular}{@{}ll@{}}
\toprule
